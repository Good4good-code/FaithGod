  ...
  return (
    <div id="daily-verse-view" className="max-w-4xl mx-auto space-y-8 py-4">
      ...
      <div className="flex items-center justify-between text-stone-500 dark:text-slate-400">
        ...
        <span className="text-xs bg-[#0f172a]/10 dark:bg-[#d4af37]/10 text-[#0f172a] dark:text-[#d4af37] px-2.5 py-1 rounded-full border border-slate-200 dark:border-slate-800/80 font-medium">
          {verse.theme}
        </span>
      </div>

      {/* Main Verse Card */}
      <div className="relative bg-white dark:bg-[#0f172a] border border-[#e7e5e4] dark:border-slate-800 rounded-3xl p-8 sm:p-12 text-center shadow-md overflow-hidden">
        {/* Background Decorative Graphic */}
        <div className="absolute -top-10 -left-10 text-stone-50/70 dark:text-slate-800/20 select-none pointer-events-none">
          <Quote className="w-48 h-48 transform -rotate-12" />
        </div>

        <div className="relative z-10 space-y-6 max-w-2xl mx-auto">
          <p className="text-2xl sm:text-3xl md:text-4xl text-stone-800 dark:text-slate-100 font-serif leading-relaxed italic">
            "{verse.verse}"
          </p>
          
          <p className="text-sm sm:text-base font-bold text-stone-500 dark:text-slate-400 font-mono tracking-wide">
            — {verse.reference}
          </p>
          ...
          <div className="pt-6 flex flex-wrap gap-3 justify-center items-center">
            <button
              ...
              className={`px-5 py-2.5 rounded-xl border text-sm font-semibold flex items-center gap-2 transition-all cursor-pointer ${
                isFavorited
                  ...
                  : 'bg-white dark:bg-[#131f42] border-stone-200 dark:border-slate-800 text-stone-600 dark:text-slate-300 hover:bg-stone-50 dark:hover:bg-slate-800'
              }`}
            >
            ...
            <button
              id="btn-copy-today"
              onClick={handleCopy}
              className="px-5 py-2.5 rounded-xl border border-stone-200 dark:border-slate-800 bg-white dark:bg-[#131f42] hover:bg-stone-50 dark:hover:bg-slate-800 text-stone-600 dark:text-slate-300 text-sm font-semibold flex items-center gap-2 transition-all cursor-pointer"
            >
            ...
            <button
              ...
              disabled={loading}
              className="px-5 py-2.5 rounded-xl bg-[#0f172a] hover:bg-[#131f42] text-[#d4af37] text-sm font-semibold flex items-center gap-2 transition-all shadow-sm hover:shadow active:scale-95 disabled:opacity-50 cursor-pointer border border-[#d4af37]/20"
            >
      ...
      {/* Devotional Output */}
      {loading && (
        <div className="bg-white dark:bg-[#0f172a] border border-stone-200 dark:border-slate-800 rounded-3xl p-8 text-center space-y-4 shadow-sm animate-pulse">
          <Sparkles className="w-8 h-8 text-[#d4af37] animate-spin mx-auto" />
          <h3 className="text-lg font-serif font-medium text-stone-800 dark:text-slate-100">Writing Biblical Devotion...</h3>
          <p className="text-xs text-stone-400 dark:text-slate-400 max-w-sm mx-auto leading-relaxed">
      ...
      {!loading && (parsedReflection || reflection) && (
        <div className="bg-[#fafaf9] dark:bg-[#0f172a] border border-[#e7e5e4] dark:border-slate-800 rounded-3xl p-8 sm:p-10 shadow-sm space-y-8 animate-fadeIn">
          <div className="flex items-center justify-between border-b border-stone-200 dark:border-slate-800 pb-4">
            <h3 className="text-xl font-bold text-stone-800 dark:text-slate-100 font-serif flex items-center gap-2">
            ...
            <div className="flex gap-2">
              <button
                onClick={handleCopy}
                className="p-2 text-stone-500 dark:text-slate-400 hover:text-stone-800 dark:hover:text-slate-200 bg-white dark:bg-[#131f42] border border-stone-200 dark:border-slate-800 rounded-lg hover:bg-stone-50 dark:hover:bg-slate-800 transition-colors"
          ...
          {parsedReflection ? (
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <span className="text-xs font-bold uppercase tracking-widest text-[#0f172a] dark:text-[#d4af37] font-mono">Theological Reflection</span>
                <div className="prose prose-stone dark:prose-invert text-stone-700 dark:text-slate-200 leading-relaxed text-sm md:text-base font-serif whitespace-pre-line">
                  {parsedReflection.reflectionText}
                </div>
              </div>
              ...
              {parsedReflection.applications.length > 0 && (
                <div className="bg-white dark:bg-[#131f42] border border-stone-100 dark:border-slate-800/60 p-6 rounded-2xl space-y-4">
                  <span className="text-xs font-bold uppercase tracking-widest text-[#0f172a] dark:text-[#d4af37] font-mono">Daily Faith Application</span>
                  <ul className="space-y-3">
                    {parsedReflection.applications.map((app, idx) => (
                      <li key={idx} className="flex items-start gap-3 text-sm text-stone-600 dark:text-slate-300">
                        <span className="w-5 h-5 rounded-full bg-[#0f172a]/10 dark:bg-[#d4af37]/15 text-[#d4af37] font-bold text-xs flex items-center justify-center shrink-0 mt-0.5">
                          {idx + 1}
                        </span>
                        <span>{app}</span>
                      </li>
              ...
              {/* Prayer */}
              <div className="bg-slate-50 dark:bg-slate-900/40 border border-slate-200/60 dark:border-slate-800/80 p-6 rounded-2xl space-y-3 italic">
                <span className="text-xs font-bold uppercase tracking-widest text-[#0f172a] dark:text-[#d4af37] font-mono not-italic block">Heartfelt Prayer</span>
                <p className="text-stone-700 dark:text-slate-300 leading-relaxed font-serif text-sm sm:text-base">
                  {parsedReflection.prayer}
                </p>
              </div>
            </div>
            ...
            <div className="prose prose-stone dark:prose-invert text-stone-700 dark:text-slate-200 leading-relaxed font-serif whitespace-pre-line text-sm">
              {reflection}
            </div>
          )}
          ...
          <div className="pt-4 border-t border-stone-200 dark:border-slate-800 flex justify-between items-center text-xs text-stone-400 dark:text-slate-500 font-mono">
            ...
            <button
              ...
              className="text-[#d4af37] hover:text-[#0f172a] dark:hover:text-[#d4af37] hover:underline font-semibold bg-transparent border-none cursor-pointer"
            >
